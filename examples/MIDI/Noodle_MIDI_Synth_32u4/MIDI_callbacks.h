...
#   endif
    ...
    int freq = (int)synthEngine::getNoteAsFrequency(mixer.getNote(i));
    int lowerBound = freq >> 2;
    int upperBound = freq << 1;
    freq = map(tempData[mixer.getChannel(i)].bendFactor, -8191, 8191, lowerBound, upperBound);
...
void instrumentChange(byte channel, byte control, byte instrument) {
#if defined(DEBUG)
  Serial.print("channel: ");
  Serial.print(channel);
...
void pitchChange(byte channel, byte control, byte bend) {
#if defined(DEBUG)
  Serial.print("channel: ");
  Serial.print(channel);
  ...
  for (int i = 0; i < NUM; i++) {
    if (mixer.getChannel(i) == channel) {
      ...
      int lowerBound = freq >> 2;
      int upperBound = freq << 1;
      freq = map(bend, -8191, 8191, lowerBound, upperBound);
      mixer.setFrequency(i, freq);
      tempData[channel].bendFactor = bend;
    }
  }
#endif
}
